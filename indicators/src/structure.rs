        ...
        if let Some(prev) = &self.prev_candle {
            ...
            if self.count < self.period {
                ...
                if self.count == self.period {
                    ...
                    let pdi = if tr_sum == 0.0 {
                        0.0
                    } else {
                        pdm_sum / tr_sum * 100.0
                    };
                    let mdi = if tr_sum == 0.0 {
                        0.0
                    } else {
                ...
                self.plus_di = Some(
                    (self.plus_di.unwrap() * (self.period as f64 - 1.0)
                        + plus_dm * 100.0 / if ts == 0.0 { 1.0 } else { ts })
                        / self.period as f64,
                );
                self.minus_di = Some(
                    (self.minus_di.unwrap() * (self.period as f64 - 1.0)
                        + minus_dm * 100.0 / if ts == 0.0 { 1.0 } else { ts })
                        / self.period as f64,
                );
...
impl NetOpenInterest {
    pub fn update(volume_delta: f64, oi_delta: f64) -> NetOiOutput {
        ...
        NetOiOutput {
            taker_long: taker_long.abs(),
            taker_short: taker_short.abs(),
            volume_delta,
            oi_delta,
        }
    }
...
mod tests {
    ...
    fn adx_basic() {
        ...
        assert!(adx
            .update(&Candle::new(6.0, 7.0, 5.0, 6.0, 100.0))
            .is_some());
    }
    ...
    fn adx_constant_prices() {
        let mut adx = Adx::new(3);
        for _ in 0..6 {
            adx.update(&Candle::new(10.0, 10.0, 10.0, 10.0, 100.0));
        }
